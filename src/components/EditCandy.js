...
const EditCandy = () => {
  ...
  function UpdateCandy() {
    if (id != "" && name != "" && wrapColor != "" && flavor != "" && amount != "" && type != "") {
      let data = { Name: name, WrapColor: wrapColor, Flavor: flavor, Amount: amount, Type: type }
      fetch(`http://localhost:5000/updateCandy/${id}?Key=${key}`, {
        method: 'PUT',
  ...
  return (
    <div className='EditCandy'>
      ...
      <div>
        ...
        <button onClick={() => { UpdateCandy(); }}>Update!</button>
        <br />
      </div>
      <div>{apiRespose}</div>
    </div>
